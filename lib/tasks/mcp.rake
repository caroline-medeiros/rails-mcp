# frozen_string_literal: true

# lib/tasks/mcp.rake
require 'json'
require 'fileutils'

namespace :mcp do
  desc 'Setup Rails MCP wrapper and inject config into Claude Desktop'
  task :setup do
    wrapper_path = File.join(Dir.home, 'rails-mcp-wrapper.sh')
    claude_config_path = File.expand_path('~/Library/Application Support/Claude/claude_desktop_config.json')

    current_app_name = File.basename(Dir.pwd)
    current_app_path = Dir.pwd

    # ==========================================================================
    # WRAPPER V3: DETECÇÃO INTELIGENTE POR ARQUIVO
    # ==========================================================================
    wrapper_content = <<~SCRIPT
      #!/bin/bash
      # Wrapper Rails MCP - V3 (File Based Detection)

      LOGfile="$HOME/mcp-debug.log"
      exec 2>> "$LOGfile"

      PROJECT_PATH="$1"
      export HOME=$(echo ~)
      export LANG="en_US.UTF-8"
      export LC_ALL="en_US.UTF-8"

      # --- SILENCIADORES ---
      export DISABLE_SPRING=1
      export RAILS_LOG_TO_STDOUT=false
      export BUNDLE_SILENCE_ROOT_WARNING=1

      cd "$PROJECT_PATH" || { echo "FATAL: Falha ao acessar $PROJECT_PATH" >&2; exit 1; }

      MANAGER_FOUND="não"

      # ========================================================================
      # ESTRATÉGIA 1: DETECÇÃO POR ARQUIVO (Prioridade Máxima)
      # ========================================================================

      # Se tem .tool-versions, provavelmente é ASDF ou MISE
      if [ -f ".tool-versions" ]; then
        if [ -f "$HOME/.local/bin/mise" ] || command -v mise >/dev/null 2>&1; then
           export PATH="$HOME/.local/bin:$PATH"
           eval "$(mise activate bash)"
           MANAGER_FOUND="mise (via .tool-versions)"
        elif [ -f "$HOME/.asdf/asdf.sh" ]; then
           . "$HOME/.asdf/asdf.sh"
           MANAGER_FOUND="asdf (via .tool-versions)"
        elif [ -f "/opt/homebrew/opt/asdf/libexec/asdf.sh" ]; then
           . "/opt/homebrew/opt/asdf/libexec/asdf.sh"
           MANAGER_FOUND="asdf (via .tool-versions)"
        fi
      fi

      # Se tem .ruby-version e ainda não achou nada, TENTA RBENV PRIMEIRO
      if [ "$MANAGER_FOUND" = "não" ] && [ -f ".ruby-version" ]; then
        if [ -d "$HOME/.rbenv" ]; then
          export PATH="$HOME/.rbenv/bin:$HOME/.rbenv/shims:$PATH"
          if command -v rbenv > /dev/null; then
            eval "$(rbenv init - bash)"
            MANAGER_FOUND="rbenv (via .ruby-version)"
          fi
        fi
      fi

      # ========================================================================
      # ESTRATÉGIA 2: FALLBACK (Se não tiver arquivos de config)
      # ========================================================================

      # Tenta carregar ASDF se existir (Padrão antigo)
      if [ "$MANAGER_FOUND" = "não" ]; then
        if [ -f "$HOME/.asdf/asdf.sh" ]; then . "$HOME/.asdf/asdf.sh"; MANAGER_FOUND="asdf (fallback)"; fi
        if [ -f "/opt/homebrew/opt/asdf/libexec/asdf.sh" ]; then . "/opt/homebrew/opt/asdf/libexec/asdf.sh"; MANAGER_FOUND="asdf (fallback)"; fi
      fi

      # Tenta carregar RBENV se existir
      if [ "$MANAGER_FOUND" = "não" ]; then
        if [ -d "$HOME/.rbenv" ]; then
          export PATH="$HOME/.rbenv/bin:$HOME/.rbenv/shims:$PATH"
          if command -v rbenv > /dev/null; then eval "$(rbenv init - bash)"; MANAGER_FOUND="rbenv (fallback)"; fi
        fi
      fi

      # Tenta RVM
      if [ "$MANAGER_FOUND" = "não" ]; then
        if [ -s "$HOME/.rvm/scripts/rvm" ]; then source "$HOME/.rvm/scripts/rvm"; rvm use . > /dev/null; MANAGER_FOUND="rvm"; fi
      fi

      echo "Gerenciador selecionado: $MANAGER_FOUND" >&2

      # EXECUÇÃO
      bundle check > /dev/null 2>&1 || bundle install > /dev/null 2>&1

      if [ -f "bin/rails-mcp" ]; then
        exec bin/rails-mcp
      else
        exec bundle exec rails-mcp
      fi
    SCRIPT

    File.write(wrapper_path, wrapper_content)
    FileUtils.chmod('+x', wrapper_path)
    puts "✅ Script wrapper atualizado (V3) em: #{wrapper_path}"

    FileUtils.mkdir_p(File.dirname(claude_config_path))
    if File.exist?(claude_config_path)
      begin
        config_data = JSON.parse(File.read(claude_config_path))
      rescue JSON::ParserError
        config_data = {}
      end
    else
      config_data = {}
    end

    config_data['mcpServers'] ||= {}
    config_data['mcpServers'][current_app_name] = {
      'command' => wrapper_path,
      'args' => [current_app_path]
    }

    File.write(claude_config_path, JSON.pretty_generate(config_data))
    puts "Claude configurado para: #{current_app_name}"
    puts 'Reinicie o Claude Desktop.'
  end
end
